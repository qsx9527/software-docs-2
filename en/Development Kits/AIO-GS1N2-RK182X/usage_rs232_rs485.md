...
RK182X开发套件 has a RS485 interface, the device name is `/dev/ttyS2`, supports full duplex, and the default baud rate is `9600`.
...
In addition to RS485, RK182X开发套件 also has an RS232 interface, the device name is `/dev/ttyS1`, supports full duplex, and the default baud rate is `115200`.
