...
GS1-N2 supports wireless bluetooth, you can display the bluetooth device information through `hciconfig -a` command:
...
(5) At this point, the GS1-N2 Bluetooth device can be found on the smartphone, and the phone can be paired after clicking:
...
GS1-N2 does not have this tool installed by default, and users need to compile and install it by themselves. Here are the steps for version 1.3.0, as follows:
...
The Obex service is needed in Linux. Firstly, connect the GS1-N2 to the Bluetooth device according to the previous steps, then start the Obex daemon, and set the receiving directory to `/home/linaro/`:
...
The following demonstrates the steps for GS1-N2 to receive files:
...
(3) Connect GS1-N2 (master device):
...
(5) Check the `test.txt` file in the `/home/linaro/` directory of GS1-N2:
