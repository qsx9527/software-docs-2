...
Linux has its own LED subsystem for LED devices. In CORE-1126BJD4, LEDs are configured as LED class devices.You can control them via `/sys/class/leds/`.
...
The board-level configuration is located in `sdk/kernel-6.1/arch/arm64/boot/dts/rockchip/rv1126b-firefly-aio-1126bjd4.dtsi`：
...
--->LED Support
   --->LED Trigger support
      --->LED Timer Trigger
```
Save the configuration and compile the kernel, the `kernel.img` burn CORE-1126BJD4 board. We can use the serial input command, you can see the blue light non-stop interval flashing.
