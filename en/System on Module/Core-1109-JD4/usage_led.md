...
Linux has its own LED subsystem for LED devices. In Core-1109-JD4, LEDs are configured as LED class devices.You can control them via `/sys/class/leds/`.
...
--->LED Support
   --->LED Trigger support
      --->LED Timer Trigger
```
Save the configuration and compile the kernel, and burn `zboot.img` to the Core-1109-JD4 board. We can use the serial port to input commands, and we can see the corresponding lights flashing at intervals.
