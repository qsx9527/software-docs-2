...
CAM-C1109S2U have 5  GPIO bank：GPIO0~GPIO4，Each group was numbered A0~A7, B0~B7, C0~C7, and D0~D7, the following formulas are often used to calculate GPIO Pin:
...
......
```
When the GPIO3_D6 pin is not reused by other peripherals, we can
We need to confirm that the GPIO is not occupied and other functions are not initialized. If GPIO3_D6 is also the I2S_m0 port, you need to disable I2S in the kernel. Add the following code at the end of /path/to/sdk/kernel/arch/arm/boot/dts/rv1109-firefly-ai-cam.dts (rv1126-firefly-ai-cam.dts):
```
&i2s0 {
        status = "disabled";
};
```
After adding, you need to compile and burn the kernel.

After completing the above operations. You can export the pin to use GPIO
```bash
