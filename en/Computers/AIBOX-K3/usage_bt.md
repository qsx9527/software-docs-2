...
(5) At this point, the AIBOX-K3 Bluetooth device can be found on the smartphone, and the phone can be paired after clicking:
...
AIBOX-K3 does not have this tool installed by default, and users need to compile and install it by themselves. Here are the steps for version 1.3.0, as follows:
...
The Obex service is needed in Linux. Firstly, connect the AIBOX-K3 to the Bluetooth device according to the previous steps, then start the Obex daemon, and set the receiving directory to `/home/linaro/`:
...
The following demonstrates the steps for AIBOX-K3 to receive files:
...
(3) Connect AIBOX-K3 (master device):
...
(5) Check the `test.txt` file in the `/home/linaro/` directory of AIBOX-K3:
