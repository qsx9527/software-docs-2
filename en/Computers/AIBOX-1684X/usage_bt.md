...
(5) At this point, the AIBOX-1684X Bluetooth device can be found on the smartphone, and the phone can be paired after clicking:
...
The Obex service is needed in Linux. Firstly, connect the AIBOX-1684X to the Bluetooth device according to the previous steps, then start the Obex daemon, and set the receiving directory to `/home/linaro/`:
...
The following demonstrates the steps for AIBOX-1684X to receive files:
...
(3) Connect AIBOX-1684X (master device):
...
(5) Check the `test.txt` file in the `/home/linaro/` directory of AIBOX-1684X:
