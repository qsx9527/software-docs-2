---
title: "Bluetooth"
description: "AIBOX-1688 Bluetooth documentation."
---

# Bluetooth

The AIBOX-1688 supports wireless Bluetooth and can display Bluetooth device information via the `hciconfig -a` command:
...
（5）At this point the AIBOX-1688 Bluetooth device can be found on the smartphone, and the phone can be paired up by clicking on it:
...
AIBOX-1688 does not have this tool installed by default, users need to compile and install it by themselves, here is a demo of the steps for version 1.3.0, as follows:
...
The Obex service is required in Linux. First, AIBOX-1688 connects the Bluetooth device according to the previous steps, then starts the Obex daemon and sets the receiving directory to `/home/linaro/`:
...
The following demonstrates the steps for the AIBOX-1688 to receive documents:
...
（3）Connect the AIBOX-1688 (master):
...
（5）Checking the `/home/linaro/` directory of AIBOX-1688 has the file `test.txt`:
