...
![](../../../rv1126_img/linux_buildroot_menuconfig.png)

我们可以在配置界面添加或裁剪一些工具，按需求定制系统功能。以添加 `qt53d` 为例：

输入 `/` 进入搜索界面，输入要查找的内容 `qt53d`，按回车进行搜索：

![](../../../rv1126_img/linux_buildroot_serach_qt53d.png)

![](../../../rv1126_img/linux_buildroot_qt53d.png)

选择 `1` 跳转到对应页面，按空格选中配置：

![](../../../rv1126_img/linux_buildroot_select_qt53d.png)

配置完成后，移动到 `Save` 按回车保存到 `.config`；移动到 `Exit` 按回车退出。

![](../../../rv1126_img/linux_buildroot_save_exit.png)
...
![](../../../rv1126_img/linux_buildroot_busybox_menuconfig.png)

配置完成后，移动到 `Exit` 按回车退出，在弹窗页面选择 `Yes` 保存到 `.config`。

![](../../../rv1126_img/linux_buildroot_busybox_save.png)
...
打开配置菜单 `make menuconfig`，通过设置 `BR2_ROOTFS_OVERLAY` 选项，添加用于覆盖的根目录。对于 RV1126_RV1109，默认已添加了目录 `board/rockchip/rk356x/fs-overlay/`。
