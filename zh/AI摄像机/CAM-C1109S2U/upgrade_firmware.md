...
本文介绍了如何将主机上的固件，通过TYPE-C 数据线烧录到 CAM-C1109S2U 开发板的存储器中。升级时，需要根据主机操作系统和固件类型来选择合适的升级方式。

## 准备工具
* CAM-C1109S2U 开发板
* [固件](https://www.t-firefly.com/doc/download/98.html)
* 主机
* 良好的TYPE-C 数据线

## 准备固件
固件可以通过编译SDK获得，也可以通过[资源下载](https://www.t-firefly.com/doc/download/98.html)处下载公版固件（统一固件）。固件文件一般有两种：
