...
## GPIO引脚计算
CAM-C1126S2U 有 5 组 GPIO bank：GPIO0~GPIO4，每组又以 A0~A7, B0~B7, C0~C7, D0~D7 作为编号区分，常用以下公式计算引脚：
...
```
下面演示 GPIO3_D6 pin脚计算方法：

bank = 3;  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;//GPIO<font color=red>3</font>_D6 => 3,  bank ∈ [0,4]

group = 3;  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;//GPIO3_<font color=red>D</font>6 => 3,  group ∈ {(A=0), (B=1), (C=2), (D=3)}

X = 6;    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;//GPIO3_D<font color=red>6</font> => 6, X ∈ [0,7]

number = group * 8 + X = 3 * 8 + 6 = 30

pin = bank*32 + number= 3 * 32 + 30 = 126;

GPIO3_D6 对应的设备树属性描述为:<&gpio3 30 GPIO_ACTIVE_HIGH>,由`kernel/include/dt-bindings/pinctrl/rockchip.h`的宏定义可知，也可以将GPIO3_D6 描述为<&gpio3 RK_PD6 GPIO_ACTIVE_HIGH>。
...
...
```
当 GPIO3_D6 脚没有被其它外设复用时，我们可以
需要确认该 GPIO 没有被占用没有被初始化其他功能。如 GPIO3_D6 同时也是 I2S_m0 口，需要在内核中关闭 I2S 在 /path/to/sdk/kernel/arch/arm/boot/dts/rv1109-firefly-ai-cam.dts (rv1126-firefly-ai-cam.dts) 末尾添加以下代码：
```
&i2s0 {
        status = "disabled";
};
...
完成上述操作后。可以通过 export 导出该引脚去使用 GPIO
```bash
...
[root@RV1126_RV1109:/]# echo 126 > /sys/class/gpio/export
[root@RV1126_RV1109:/]# ls /sys/class/gpio/
export  gpio126  gpiochip0  gpiochip128  gpiochip32  gpiochip503  gpiochip511  gpiochip64  gpiochip96  unexport
[root@RV1126_RV1109:/]# ls /sys/class/gpio/gpio126
...
```shell
[root@RV1126_RV1109:/]# cat /sys/class/gpio/gpio126/direction
in
```
...
```shell
[root@RV1126_RV1109:/]# echo in > /sys/class/gpio/gpio126/direction
```

* 在输入模式下，直接查看 value 文件的值即可知道输入的电平

```shell
[root@RV1126_RV1109:/]# cat /sys/class/gpio/gpio126/value
0
```
...
```shell
[root@RV1126_RV1109:/]# echo out > /sys/class/gpio/gpio126/direction
```

* 设置输出高电平

```shell
[root@RV1126_RV1109:/]# echo 1 > /sys/class/gpio/gpio126/value
```

* 设置输出低电平

```shell
[root@RV1126_RV1109:/]# echo 0 > /sys/class/gpio/gpio126/value
```
...
GPIO 调试有一个很好用的工具，那就是 IO 指令，CAM-C1126S2U 的 Linux 系统默认已经内置了 IO 指令，使用 IO 指令可以实时读取或写入每个 IO 口的状态，这里简单介绍 IO 指令的使用。首先查看 IO 指令的帮助：
