---
title: "快速部署 Llama3"
description: "AIO-1684XJD4 快速部署 Llama3文档。"
---
...
将部署包传输至 AIO-1684XJD4 机器，推荐使用 `scp` 命令传输：

1. 在 AIO-1684XJD4 上执行 `ifconfig` 命令查看其 IP 地址。
2. 输入 `scp` 命令传输部署包，例：`scp llama3.tar linaro@<aibox-ip-addr>:~/`，其中 `<aibox-ip-addr>` 为 AIO-1684XJD4 的 IP 地址。
