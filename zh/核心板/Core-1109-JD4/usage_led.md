...
标准的 Linux 专门为 LED 设备定义了 LED 子系统。 在 Core-1109-JD4开发板中的LED 均以设备的形式被定义。用户可以通过 `/sys/class/leds/` 目录控制LED。
...
--->LED Support
   --->LED Trigger support
      --->LED Timer Trigger
```
保存配置并编译内核，把 `zboot.img` 烧到 Core-1109-JD4板子上 我们可以使用串口输入命令，就可以看到对应的灯不停的间隔闪烁。
