...
（5）此时在智能手机就能发现 AIBOX-K3 蓝牙设备，手机点击后就能配对上去：
...
AIBOX-K3 默认没有安装该工具，用户需要自行编译安装，这里演示 1.3.0 版本的步骤，如下：
...
在 Linux 中需要用到 Obex 服务，首先 AIBOX-K3 根据前面的步骤连接好蓝牙设备，然后开启 Obex 守护进程，并设置接收的目录为 `/home/linaro/`：
...
（5）查看 AIBOX-K3 的 `/home/linaro/`目录有 `test.txt` 文件：
